...
# The free DuckDuckGo package was renamed from `duckduckgo_search` to `ddgs`.
# Try the new name first and fall back to the old one so the app keeps working
...
def duckduckgo_search(
    ...
    """
    # DuckDuckGo's free endpoint frequently rate-limits back-to-back requests
    # (it returns an empty list or raises). So we try a few times with a growing
    # pause before giving up — this makes searches succeed far more often.
    import time
    ...
    for attempt in range(attempts):
        try:
            with DDGS() as ddgs:
                ...
                # Empty result set is usually rate-limiting — retry below.
                print(f"DuckDuckGo returned no results (attempt {attempt + 1}/{attempts}).")
        except Exception as e:
            print(f"Error in DuckDuckGo search (attempt {attempt + 1}/{attempts}): {str(e)}")
            print(f"Full error details: {type(e).__name__}")

        # Wait before the next try (2s, 4s, 6s), but not after the final attempt.
        if attempt < attempts - 1:
            time.sleep((attempt + 1) * 2)

    # All attempts failed - return empty so the rest of the app can continue.
    return {"results": []}
