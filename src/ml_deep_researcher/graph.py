...
from ml_deep_researcher.configuration import Configuration, SearchAPI
from ml_deep_researcher.utils import (
...
from ml_deep_researcher.state import (
    SummaryState,
    SummaryStateInput,
    SummaryStateOutput,
)
from ml_deep_researcher.prompts import (
...
from ml_deep_researcher.lmstudio import ChatLMStudio
...
def web_research(state: SummaryState, config: RunnableConfig):
    ...
    if search_api == "tavily":
        ...
        # MY CUSTOMIZATION: I raised max_results from 3 -> 4 for DuckDuckGo.
        # ML/DL topics often need one extra source to cover both the intuition
        # and the implementation detail, and DuckDuckGo is free so this costs
        # nothing. Sources are de-duplicated later, so a small bump is safe.
...
def finalize_summary(state: SummaryState):
    """LangGraph node that finalizes the research into STUDY NOTES.

    MY CUSTOMIZATION:
    The original project just wrapped the summary under a "## Summary" heading
    followed by sources. I redesigned the final report so it reads like a tidy
    set of study notes for an ML/DL learner. The report now has:
      - a clear title with the topic,
      - the main study notes (the iteratively-built summary),
      - a small auto-generated "Suggested follow-up study questions" section
        (built from the topic so the student knows what to revise next),
      - a clean, de-duplicated list of sources.
    ...
    Returns:
        ...
        formatted final study-note report.
    """

    # Deduplicate sources before joining (same robust logic as the original).
    seen_sources = set()
    unique_sources = []
    ...
    topic = state.research_topic or "your topic"
    loops = state.research_loop_count

    # A few generic-but-useful revision prompts, tailored with the topic name.
    follow_up_questions = (
        f"- What is the core intuition behind **{topic}**, in one or two sentences?\n"
        f"- What problem does **{topic}** actually solve, and what did people do before it?\n"
        f"- What are the most common mistakes or misconceptions about **{topic}**?\n"
        f"- How would you explain **{topic}** to a fellow student in an interview?\n"
    )

    # Assemble the final study-note report.
    state.running_summary = (
        f"# 📚 ML/DL Study Notes: {topic}\n\n"
        f"_Generated by my ML & Deep Learning Research Agent after {loops} research cycle(s)._\n\n"
        f"## 🧠 Study Notes\n"
        f"{state.running_summary}\n\n"
        f"## ❓ Suggested follow-up study questions\n"
